...
class DefaultHead(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, code_size,
                 predict_boxes_when_training=True, num_small_filters=64):
        super().__init__(
            model_cfg=model_cfg, num_class=num_class, class_names=class_names, grid_size=grid_size, point_cloud_range=point_cloud_range,
            predict_boxes_when_training=predict_boxes_when_training, num_small_filters=num_small_filters)
...
class SmallHead(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, code_size,
                 predict_boxes_when_training=True, num_small_filters=64):
        super().__init__(
            model_cfg=model_cfg, num_class=num_class, class_names=class_names, grid_size=grid_size, point_cloud_range=point_cloud_range,
        ...
        self.net = nn.Sequential(
            nn.Conv2d(num_small_filters, input_channels, 3, bias=False, padding=1),
    ...
    def forward(self, data_dict):
        ret_dict = {}
        spatial_features_2d = data_dict['multi_small_input']
...
class AnchorHeadMultiDis(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        self.large_head = DefaultHead(model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, code_size,
                                      predict_boxes_when_training=predict_boxes_when_training, num_small_filters=num_small_filters)
        self.small_head = SmallHead(model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, code_size,
                                    predict_boxes_when_training=predict_boxes_when_training, num_small_filters=num_small_filters)
        self.rpn_heads = nn.ModuleList([self.large_head, self.small_head])
        self.model_cfg = model_cfg
        self.use_multihead = self.model_cfg.get('USE_MULTIHEAD', False)

    def forward(self, data_dict):
        ret_dicts = []
        for rpn in self.rpn_heads:
            ret_dicts.append(rpn(data_dict))
        # ret_dicts.append(self.large_head(data_dict))
        # ret_dicts.append(self.small_head(data_dict))
        ...
        if self.model_cfg.get('USE_DIRECTION_CLASSIFIER', False):
            ...
            ret['dir_cls_preds'] = torch.cat(dir_cls_preds, dim=1)
