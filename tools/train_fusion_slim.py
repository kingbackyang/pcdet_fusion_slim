...
def parse_config():
    ...
    parser.add_argument('--slimming_pth', default="", help='the dir to save logs and models')
...
def main():
    ...
    if cfg.MODEL.BACKBONE_2D.get('FUSION_VOXEL_SPLIT', None) is not None:
        ...
        cfg.MODEL.BACKBONE_2D.FUSION_VOXEL_SPLIT = 1
    ...
    for i in range(1, 4):
        attr_name = f"block{i}"
        module_part = getattr(model.backbone_2d, attr_name)
        count = 0
        for m in module_part:
            if isinstance(m, nn.BatchNorm2d):
    ...
    for key in depth_dict.keys():
        ...
        tmp.sort()
        depth_dict[key] = tmp
    with open(f"/home/mk/OpenPCDet/benchmark/slim_fusion_description/slim_description_{args.slim_ratio}.json", "w") as f:
    ...
    for i in range(1, 4):
        ...
        for m in module_part:
            if isinstance(m, nn.ConvTranspose2d):
                m.weight.data = m.weight.data[deconv_cfg_list[i-1], :, :, :]
                key = rpn_key_list[signal]
                de_mask_cfg = rpn_dict[key]
                if signal == 0:
                    mask_total = 0
                else:
                    mask_total = sum(rpn_list[:int(signal)])
                if len(de_mask_cfg) == 0:
                    de_mask_cfg.append(mask_total)
                    de_mask_cfg.append(int(mask_total + 1))
                    rpn_dict[key] = de_mask_cfg
                de_mask_cfg = torch.tensor(de_mask_cfg) - mask_total
                neck_list.append(de_mask_cfg)
                rpn_conv_length = rpn_list[signal]

                mask = torch.zeros(rpn_conv_length)
                de_mask_cfg = de_mask_cfg.clone().detach()
                mask[de_mask_cfg] = 1
                m.weight.data = m.weight.data[:, de_mask_cfg, :, :]
                print("signal: {} conv shape: {}".format(signal, m.weight.data.shape))
                if m.bias.data.shape[0] != 0:
                    m.bias.data = m.bias.data[de_mask_cfg]
                signal = signal + 1

            if isinstance(m, nn.BatchNorm2d):
                de_mask_cfg = list(de_mask_cfg.numpy())
                m.weight.data = m.weight.data[de_mask_cfg]
                m.bias.data = m.bias.data[de_mask_cfg]
                m.running_mean = m.running_mean[de_mask_cfg]
                m.running_var = m.running_var[de_mask_cfg]
                print("signal: {} bn shape: {}".format(signal, m.weight.data.shape[0]))
    channels_mile = [int(sum(channels_list[:int(i + 1)])) for i in range(len(channels_list[:3]))]
    ...
    torch.save(model_slim.state_dict(),
               f"/home/mk/OpenPCDet/benchmark/slim_fusion_description/slim_model_{args.slim_ratio}.pth")
