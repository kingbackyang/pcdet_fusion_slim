...
def paint_voxel_reverse_kernel(voxels,
                                num_points,
                                coordinates,
                                voxel_size,
                                coors_range,
                                voxelmap_shape,
                                bev_map,
                                channels_per_voxel=3):
    # put all computations to one loop.
    # we shouldn't create large array in main jit code, otherwise
    # reduce performance
    voxel_scores = np.zeros(shape=(channels_per_voxel,), dtype=np.float32)
    num_voxels = voxels.shape[0]
    for i in range(num_voxels):
        points_in_voxel = voxels[i, 0:num_points[i], :]  # shape (num, 8)
        z_idx = np.floor((points_in_voxel[:, 2] - coors_range[2]) / voxel_size[2])
        for j in range(voxelmap_shape[0]):
            voxel_mask = z_idx == j
            points = points_in_voxel[voxel_mask]
            if points.size == 0:
                continue
            if coordinates[i][2] >=432:
                continue

            for chan in range(channels_per_voxel):
                voxel_scores[chan] = np.mean(points[:, 4])
            coor = coordinates[i]

            bev_map[channels_per_voxel * j:channels_per_voxel * (j + 1), coor[1], coor[2]] = voxel_scores


def paint_voxel(voxels,
                num_points,
                coordinates,
                voxel_size,
                coors_range,
                channels_per_voxel=4
                ):
    """

    :param voxels: [M, max_points, ndim] float tensor. only contain points.
    :param num_points: [M] int32 tensor.
    :param coordinates: [M, 3] int32 tensor.
    :param voxel_size: [3] list/tuple or array, float. xyz, indicate voxel size
    :param coors_range: [6] list/tuple or array, float. indicate voxel range.
            format: xyzxyz, minmax
    :param channels_per_voxel: int. indicate channels for each voxel.
    :return: bev_map
    """
    if not isinstance(voxel_size, np.ndarray):
        voxel_size = np.array(voxel_size, dtype=np.float32)  # [0.08, 0.08, 0.5]
    if not isinstance(coors_range, np.ndarray):
        coors_range = np.array(coors_range, dtype=np.float32)  # [0, -19.84, -2.5, 47.36, 19.84, 0.5]
    voxelmap_shape = (coors_range[3:] - coors_range[:3]) / voxel_size  # [592, 496, 6]
    voxelmap_shape = tuple(np.round(voxelmap_shape).astype(np.int32).tolist())  # (592, 496, 6)
    voxelmap_shape = voxelmap_shape[::-1]  # DHW format (6, 496, 592)
    coor_to_voxelidx = -np.ones(shape=voxelmap_shape, dtype=np.int32)  # shape (6, 496, 592)
    # coors_2d = np.zeros(shape=(max_voxels, 2), dtype=np.int32)
    bev_map_shape = list(voxelmap_shape)  # 6, 496, 592
    bev_map_shape[0] *= channels_per_voxel  # 24, 496, 592
    bev_map = np.zeros(shape=bev_map_shape, dtype=np.float32)

    if voxels.shape[2] == 5:
        paint_voxel_reverse_kernel(voxels, num_points, coordinates, voxel_size, coors_range,
                                   voxelmap_shape, bev_map, channels_per_voxel)
    return bev_map
...
class BaseBEVBackboneFusion(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        if self.model_cfg.get('FUSION_VOXEL_SPLIT', None) is not None:
        ...
        if self.model_cfg.get('UPSAMPLE_STRIDES', None) is not None:
            print(len(self.model_cfg.UPSAMPLE_STRIDES))
            print(len(self.model_cfg.NUM_UPSAMPLE_FILTERS))
            assert len(self.model_cfg.UPSAMPLE_STRIDES) == len(self.model_cfg.NUM_UPSAMPLE_FILTERS)
        ...
        for i in range(layer_nums[0]):
            ...
            self.block1.append(nn.BatchNorm2d(num_filters[0]))
            self.block1.append(nn.ReLU())
        self.block1 = nn.Sequential(*self.block1)
        self.deconv1 = nn.Sequential(
            nn.ConvTranspose2d(
                num_filters[0] + fpn_fusion * 64,
        ...
        self.block2 = [
            nn.ZeroPad2d(1),
            nn.Conv2d(
                block2_input_filters + fpn_fusion * 64,
                num_filters[1],
                3,
                stride=layer_strides[1]),
        ...
        for i in range(layer_nums[1]):
            ...
            self.block2.append(nn.BatchNorm2d(num_filters[1]))
            self.block2.append(nn.ReLU())
        self.block2 = nn.Sequential(*self.block2)
        self.deconv2 = nn.Sequential(
            nn.ConvTranspose2d(
                num_filters[1] + fpn_fusion * 128,
        ...
        self.block3 = [
            nn.ZeroPad2d(1),
            nn.Conv2d(num_filters[1] + fpn_fusion * 128, num_filters[2], 3, stride=layer_strides[2]),
            nn.BatchNorm2d(num_filters[2]),
            nn.ReLU()]
        for i in range(layer_nums[2]):
            self.block3.append(
                nn.Conv2d(num_filters[2], num_filters[2], 3, padding=1))
            self.block3.append(nn.BatchNorm2d(num_filters[2]))
            self.block3.append(nn.ReLU())
        self.block3 = nn.Sequential(*self.block3)
        self.deconv3 = nn.Sequential(
            nn.ConvTranspose2d(
                num_filters[2] + fpn_fusion * 256,
...
class pointpillar(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.vfe = PillarVFE(config.MODEL.VFE, config.DATA_CONFIG.DATA_PROCESSOR[2].VOXEL_SIZE, config.DATA_CONFIG.POINT_CLOUD_RANGE, 4)
        ...
        self.backbone_2d = BaseBEVBackboneFusionSlim(config.MODEL.BACKBONE_2D, 64)
...
if __name__ == "__main__":
    import copy
    import json
    model_weights = torch.load("/home/mk/OpenPCDet/benchmark/pp_fusion_early_8_slim0.1/pointpillar_fusion_early_8/default/ckpt/checkpoint_epoch_80.pth")
    cfg_file = "/home/mk/OpenPCDet/tools/cfgs/kitti_models/pointpillar_fusion_early_8.yaml"
    slimming_json = "/home/mk/OpenPCDet/benchmark/slim_fusion_description/slim_description_0.1.json"
    ...
    cfg["MODEL"]["BACKBONE_2D"]["NAME"] = "BaseBEVBackboneFusionSlim"
    ...
    for i in range(1, len(layer_num_plus) + 1):
        for j in range(layer_num_plus[i - 1]):
    ...
    cfg_rpn = copy.deepcopy(cfg)
    model = pointpillar(cfg)
    model.eval()
    model.cuda()
    model_state = model_weights["model_state"]
    model_state_rpn = copy.deepcopy(model_state)
    del_list = []
    ...
    inp_fusion = np.fromfile(str(points_path), dtype=np.float32).reshape(-1, 5)
    # inp = inp_fusion[:, :4]
    # voxelize
    # main brabch
    x = voxelization(inp_fusion)
    ...
    # score branch
    if cfg.MODEL.BACKBONE_2D.get('FUSION_VOXEL_SPLIT', None) is not None:
        fusion_voxel_split = cfg.MODEL.BACKBONE_2D.FUSION_VOXEL_SPLIT
    else:
        fusion_voxel_split = 1
    # fusion_voxel_split = cfg.MODEL.BACKBONE_2D.FUSION_VOXEL_SPLIT
    voxel_size = cfg.DATA_CONFIG.DATA_PROCESSOR[2].VOXEL_SIZE
    voxel_size[2] = voxel_size[2] / fusion_voxel_split
    point_cloud_range = cfg.DATA_CONFIG.POINT_CLOUD_RANGE
    voxels = voxels.cpu().detach().numpy()
    voxel_num_points = voxel_num_points.cpu().detach().numpy().astype(np.int)
    coors = coors.cpu().detach().numpy().astype(np.int)

    bev_map = paint_voxel(voxels, voxel_num_points, coors, voxel_size, point_cloud_range, channels_per_voxel=3)
    print(bev_map.shape)
    ...
    del_rpn_list = []
    for key in model_state_rpn.keys():
        if key.startswith("vfe"):
            del_rpn_list.append(key)
    for var in del_rpn_list:
        del model_state_rpn[var]
    rpn_model = rpn(cfg_rpn)
    rpn_model.load_state_dict(model_state_rpn)
    rpn_model.eval()
    rpn_model.cuda()
    rpn_input = torch.ones(1, 64, 496, 432).cuda()
    bev_input = torch.ones(1, fusion_voxel_split * 3, 496, 432).cuda()
    cls_preds, box_preds, dir_cls_preds  = rpn_model(rpn_input, bev_input)
    cls_preds = cls_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
    box_preds = box_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
    dir_cls_preds = dir_cls_preds.permute(0, 2, 3, 1).contiguous()

    batch_cls_preds, batch_box_preds = rpn_model.dense_head.generate_predicted_boxes(
        batch_size=1,
        cls_preds=cls_preds, box_preds=box_preds, dir_cls_preds=dir_cls_preds
    )

    torch.onnx.export(rpn_model, (rpn_input, bev_input),
