...
class BaseBEVBackbone(nn.Module):
    ...
    def forward(self, spatial_features):
        ...
        return x, data_dict["multi_small_input"]
...
class AnchorHeadMultiDisApollo(AnchorHeadTemplate):
    ...
    def forward(self, spatial_2d, multi_small_input):
        data_dict = {}
        data_dict['spatial_features_2d'] = spatial_2d
        data_dict["multi_small_input"] = multi_small_input
        ...
        for rpn_head in self.rpn_heads:
            if rpn_head.num_class == 2:
                ret_dicts.append(rpn_head(data_dict['multi_small_input']))
            else:
...
class pointpillar(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense_head = AnchorHeadMultiDisApollo(config.MODEL.DENSE_HEAD, self.input_channels, len(self.class_names), self.class_names, self.grid_size, self.point_cloud_range, num_small_filters=config.MODEL.BACKBONE_2D.NUM_FILTERS[0])
    ...
    def forward(self, points):
        ...
        x, multi_small_input = self.backbone_2d(x)
        cls_preds, box_preds = self.dense_head(x, multi_small_input)
        return cls_preds, box_preds


if __name__ == "__main__":
    import thop
    model_weights = torch.load("/home/mk/OpenPCDet/benchmark/pointpillar_multi_apollo/pointpillar_multi_apollo/default/ckpt/checkpoint_epoch_120.pth")
    cfg_file = "/home/mk/OpenPCDet/tools/cfgs/kitti_models/pointpillar_multi_apollo.yaml"
